...
export const RouteCard = ({ 
  ...
  const getCategoryIcon = () => {
    switch(question?.category) {
      case 'Temps de conduite': return '⏱️'
      case 'Pauses': return '☕'
      case 'Repos': return '🛌'
      case 'Chronotachygraphe': return '💳'
      case 'Optimisation': return '📊'
      case 'Chargement': return '📦'
      case 'Créneaux': return '🕐'
      case 'Géographie': return '🗺️'
      case 'Coûts': return '💰'
      case 'Péages': return '🛣️'
      ...
      >
        ...
        <div className="flex items-center gap-1 mt-3">
          ...
          {[...Array(3)].map((_, i) => (
            <span 
              key={i} 
              className={`text-xs ${i < question?.difficulty ? 'text-cyan-400' : 'text-slate-700'}`}
            >
              ★
            </span>
          ))}
        ...
        {showCorrectAnswer && (
          ...
          >
            <div className="flex items-start gap-2">
              <span className={`material-icons ${accentColor} text-sm mt-0.5`}>route</span>
              <div>
                <p className="text-xs text-slate-400 uppercase tracking-wider mb-1">Optimisation</p>
