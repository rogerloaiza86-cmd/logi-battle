...
export const SupplyChainCard = ({ 
  ...
  const primaryColor = isTeamA ? 'blue' : 'amber'
  ...
  // Icône selon le type de question
  const getQuestionIcon = () => {
    switch(question?.category) {
      case 'Acteurs': return '🏭'
      case 'Flux': return '📊'
      case 'Séquence': return '➡️'
      case 'Chaîne complète': return '🔗'
      case 'Documents': return '📄'
      case 'Scénario': return '🎭'
      default: return '🧩'
    }
  }
  ...
  return (
    ...
    >
      {/* Header Card */}
      <motion.div
        className={`bg-slate-800 rounded-2xl w-full p-5 border-2 transition-colors ${
      ...
      >
        ...
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-slate-400 text-xs font-bold uppercase tracking-wider">
              Supply Chain • {question?.category}
            </span>
          </div>
          {question?.context && (
        ...
        <div className="flex items-center gap-1">
          <span className="text-xs text-slate-500 mr-2">Difficulté:</span>
          {[...Array(3)].map((_, i) => (
            <span 
              key={i} 
        ...
        {isAnswering && (
          ...
          >
            <div className="flex items-start gap-2">
              <span className={`material-icons ${accentColor} text-sm mt-0.5`}>lightbulb</span>
              <div>
