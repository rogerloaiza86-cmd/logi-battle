  ...
  useEffect(() => {
    ...
    const initGame = async () => {
      try {
        ...
        const channel = gamesService.getGameChannel(id)
        if (!channel) {
          throw new Error('Le mode QR live nécessite une configuration Supabase Realtime valide.')
        }

        // Créer la partie dans la base avec le même ID que celui affiché aux joueurs.
        ...
        if (!cancelled) {
          setInitError(err.message || 'Impossible de créer la partie live.')
          setGameId(null)
    ...
    return () => {
      cancelled = true
      if (createdGameId) {
        gamesService.removeGameChannel(createdGameId)
      }
    }
  }, [])
          ...
          >
            ...
            <div className="bg-white rounded-xl p-4 w-fit mx-auto mb-6">
              ...
              {!isInitializing && initError && (
                <div className="w-48 h-48 flex items-center justify-center text-red-600 text-sm text-center p-4">
                  {initError}
                </div>
              )}
            </div>
