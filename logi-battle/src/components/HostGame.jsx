...
export const HostGame = ({ onBack, gameMode }) => {
  ...
  useEffect(() => {
    const initGame = async () => {
      try {
        ...
        const createdId = await gamesService.createGame('ÉQUIPE ALPHA', 'ÉQUIPE OMEGA', id)
        setGameId(createdId)
        gameStore.setGameId(createdId)
      } catch (err) {
  ...
  const getPlayerUrl = () => {
    ...
    return new URL(`${basePath}/join?game=${encodeURIComponent(gameId || '')}`, window.location.origin).toString()
  }
  ...
  const startGame = () => {
    if (isInitializing || initError || !gameId) return
    setGameStarted(true)
        ...
        <div className="max-w-4xl w-full grid md:grid-cols-2 gap-8">
          ...
          >
            ...
            <motion.button
              ...
              whileTap={{ scale: 0.98 }}
              onClick={startGame}
              disabled={isInitializing || initError || !gameId}
