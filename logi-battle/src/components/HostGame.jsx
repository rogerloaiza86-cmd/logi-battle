...
export const HostGame = ({ onBack, gameMode }) => {
  ...
  useEffect(() => {
    const initGame = async () => {
      try {
        ...
        setInitError("Impossible de créer la partie. Vérifiez la configuration réseau ou Supabase avant de partager le QR code.")
      } finally {
  ...
  const getPlayerUrl = () => {
    ...
    const url = new URL(`${normalizedBasePath}join`, window.location.origin)
      ...
      <main className="flex-1 flex items-center justify-center p-6">
        <div className="max-w-4xl w-full grid md:grid-cols-2 gap-8">
          ...
          >
            ...
            {isInitializing && (
              <div className="mb-6 rounded-xl border border-white/10 bg-slate-900/70 p-4 text-center text-sm text-gray-300">
            ...
            <div className="mt-4 p-3 bg-slate-900/50 rounded-lg">
              ...
              <p className="text-xs text-gray-400 truncate">{gameId ? getPlayerUrl() : 'En attente de création...'}</p>
            </div>
          </motion.div>
