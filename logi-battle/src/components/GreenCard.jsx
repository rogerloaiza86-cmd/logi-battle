...
export const GreenCard = ({ 
  ...
  const getCategoryIcon = () => {
    switch(question?.category) {
      case 'RSE': return '🤝'
      case 'Économie circulaire': return '♻️'
      case 'Logistique verte': return '🌱'
      case 'Tri sélectif': return '🗑️'
      case 'Hiérarchie': return '📊'
      case 'Réglementation': return '⚖️'
      case 'Emballages': return '📦'
      case 'Transport': return '🚛'
      case 'Carbone': return '💨'
      case 'Énergie': return '⚡'
      case 'Bâtiment': return '🏢'
      case 'Décision': return '🤔'
      default: return '🌍'
    }
  }
  ...
  return (
    ...
    >
      <motion.div
        className={`bg-slate-800 rounded-2xl w-full p-5 border-2 ${
      ...
      >
        ...
        <div className="flex items-center gap-1 mt-3">
          ...
          {[...Array(3)].map((_, i) => (
            <span 
              key={i} 
              className={`text-xs ${i < question?.difficulty ? 'text-green-400' : 'text-slate-700'}`}
            >
              ★
            </span>
          ))}
        ...
        {question?.options?.map((option, index) => (
          ...
          >
            <div className="flex items-center gap-3">
              ...
              {showCorrectAnswer && index === question.correctOption && (
        ...
        {showCorrectAnswer && (
          ...
          >
            <div className="flex items-start gap-2">
              <span className={`material-icons ${accentColor} text-sm mt-0.5`}>eco</span>
              <div>
                <p className="text-xs text-slate-400 uppercase tracking-wider mb-1">Impact environnemental</p>
