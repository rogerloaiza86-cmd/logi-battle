...
export const PlayerGame = ({ gameId, playerName, team }) => {
  ...
  useEffect(() => {
    ...
    if (channel) {
      channel.on('broadcast', { event: 'new_question' }, ({ payload }) => {
        ...
        const nextRoundTime = payload.time || 30
  ...
  const handleSubmit = () => {
    ...
    const isCorrect = 
      ...
      Number(userAnswer) === currentQuestion?.answer
  ...
  const selectedOption = userAnswer === '' ? null : Number(userAnswer)
            ...
            >
              ...
              <div className="bg-slate-800 rounded-2xl p-5 mb-4 border border-white/10">
                <span className="text-xs font-bold text-gray-500 uppercase tracking-wider">
                  {currentQuestion?.type === 'vocabulaire' ? '📚 Vocabulaire' : hasOptions ? '🧩 QCM' : '🧠 Culture Générale'}
