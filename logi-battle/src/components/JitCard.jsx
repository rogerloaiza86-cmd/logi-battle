...
export const JitCard = ({ 
  ...
  const getCategoryIcon = () => {
    switch(question?.category) {
      ...
      case 'Kitting': return '🎁'
      case 'Flux': return '🌊'
      case 'Takt Time': return '⏱️'
      case 'Jidoka': return '🤖'
      case 'Andon': return '🚦'
      case 'Poka-Yoke': return '🛡️'
      case 'SMED': return '⚡'
      case '5S': return '✨'
      case 'Stocks': return '📦'
      ...
      >
        ...
        <div className="flex items-center gap-1 mt-3">
          ...
          {[...Array(3)].map((_, i) => (
            <span 
              key={i} 
              className={`text-xs ${i < question?.difficulty ? 'text-orange-400' : 'text-slate-700'}`}
            >
              ★
            </span>
          ))}
        ...
        {isAnswering && (
          ...
          >
            <div className="flex items-start gap-2">
              <span className={`material-icons ${accentColor} text-sm mt-0.5`}>precision_manufacturing</span>
              <div>
                <p className="text-xs text-slate-400 uppercase tracking-wider mb-1">Lean Manufacturing</p>
