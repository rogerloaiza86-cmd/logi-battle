...
export const LegalCard = ({ 
  ...
  const getCategoryIcon = () => {
    switch(question?.category) {
      ...
      case 'Responsabilité': return '⚖️'
      case 'Litiges': return '⚡'
      case 'Indemnisation': return '💰'
      case 'Assurance': return '🛡️'
      case 'Incoterms': return '🌍'
      case 'RGPD': return '🔒'
      case 'E-commerce': return '🛒'
      case 'Contrats': return '📝'
      case 'CGV': return '📋'
      case 'Douanes': return '🏛️'
      default: return '⚖️'
    }
  }
      ...
      >
        ...
        <div className="flex items-center gap-1 mt-3">
          ...
          {[...Array(3)].map((_, i) => (
            <span 
              key={i} 
              className={`text-xs ${i < question?.difficulty ? 'text-yellow-400' : 'text-slate-700'}`}
            >
              ★
            </span>
          ))}
        ...
        {isAnswering && (
          ...
          >
            <div className="flex items-start gap-2">
              <span className={`material-icons ${accentColor} text-sm mt-0.5`}>gavel</span>
              <div>
                <p className="text-xs text-slate-400 uppercase tracking-wider mb-1">Aspect juridique</p>
