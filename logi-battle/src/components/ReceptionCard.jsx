...
export const ReceptionCard = ({ 
  ...
  const getCategoryIcon = () => {
    switch(question?.category) {
      case 'Contrôle visuel': return '👁️'
      case 'Contrôle quantitatif': return '🔢'
      case 'Contrôle qualité': return '✅'
      case 'Hygiène et sécurité': return '🛡️'
      case 'Rédaction des réserves': return '✍️'
      case 'Documents de transport': return '📄'
      case 'Produits dangereux': return '☠️'
      case 'Méthodes de contrôle': return '📊'
      case 'Gestion de crise': return '🚨'
      case 'Produits frais': return '🧊'
      case 'Déontologie': return '⚖️'
      case 'Calcul': return '🧮'
      default: return '🚛'
    }
  }

  // Icône selon le type de scénario
  const getScenarioIcon = () => {
    switch(question?.type) {
      case 'anomaly_identification': return '🔍'
      case 'reserve_formulation': return '📝'
      case 'document_check': return '📋'
      case 'adr_identification': return '⚠️'
  ...
  return (
    ...
    >
      ...
      <motion.div
        className={`w-full bg-slate-800 rounded-2xl p-4 border-2 ${
      ...
      >
        ...
        {question?.scenario && (
          <div className="bg-slate-900/50 rounded-xl p-4 mb-4 border border-slate-700">
            <div className="flex items-start gap-3">
              <span className="text-2xl">{getScenarioIcon()}</span>
        ...
        <div className="flex items-center gap-1 mt-3">
          ...
          {[...Array(3)].map((_, i) => (
            <span 
              key={i} 
              className={`text-xs ${i < question?.difficulty ? 'text-amber-400' : 'text-slate-700'}`}
            >
              ★
            </span>
          ))}
      ...
      <AnimatePresence>
        {isWrong && (
          ...
          >
            ...
            Anomalie non détectée ! Recommencez
          </motion.div>
        )}
      </AnimatePresence>
        ...
        {isAnswering && (
          ...
          >
            <div className="flex items-start gap-2">
              <span className={`material-icons ${accentColor} text-sm mt-0.5`}>school</span>
              <div>
                <p className="text-xs text-slate-400 uppercase tracking-wider mb-1">Correction & Conseil</p>
                <p className="text-sm text-gray-300">{question?.explanation}</p>
                {question?.mention && (
                  <p className="text-xs text-slate-500 mt-2">
                    <span className={accentColor}>Mention de réserve:</span> {question.mention}
                  </p>
                )}
              </div>
            </div>
