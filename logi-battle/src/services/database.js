...
const getSupabaseClient = () => {
  if (!supabase) {
    throw new Error('Supabase est configuré comme base active, mais les variables VITE_SUPABASE_URL et VITE_SUPABASE_ANON_KEY sont manquantes.')
  }
  return supabase
}
...
export const gamesService = {
  ...
  removeGameChannel(gameId) {
    if (!USE_SUPABASE || !localDB.channels?.[gameId]) return
    const client = getSupabaseClient()
    client.removeChannel(localDB.channels[gameId])
