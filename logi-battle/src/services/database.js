...
export const gamesService = {
  ...
  async getGame(gameId) {
    if (USE_SUPABASE) {
      const supabaseClient = getSupabaseClient()
      const { data, error } = await supabaseClient.from('games').select('*').eq('gameId', gameId).single()
      if (error) {
  ...
  async updateGameScore(gameId, teamA_score, teamB_score, rope_position) {
    ...
    if (USE_SUPABASE) {
      const supabaseClient = getSupabaseClient()
      const { error } = await supabaseClient.from('games').update(updateData).eq('gameId', gameId)
      if (error) throw error
      return true
    }
  ...
  async updateGameStatus(gameId, status) {
    if (USE_SUPABASE) {
      const supabaseClient = getSupabaseClient()
      const { error } = await supabaseClient.from('games').update({ status }).eq('gameId', gameId)
      if (error) throw error
      return true
    }
  ...
  getGameChannel(gameId) {
    if (USE_SUPABASE) {
      ...
      if (!localDB.channels[gameId]) {
        localDB.channels[gameId] = supabaseClient.channel(`game_${gameId}`)
      }
  ...
  removeGameChannel(gameId) {
    ...
    const supabaseClient = getSupabaseClient()
    supabaseClient.removeChannel(localDB.channels[gameId])
...
export const questionsService = {
  ...
  async getQuestion(questionId) {
    if (USE_SUPABASE) {
      const supabaseClient = getSupabaseClient()
      const { data, error } = await supabaseClient.from('questions').select('*').eq('id', questionId).single()
      if (error) {
