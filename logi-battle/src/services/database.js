...
const ensureSupabaseClient = () => {
  if (!supabase) {
    throw new Error('Supabase est configuré comme base de données, mais VITE_SUPABASE_URL ou VITE_SUPABASE_ANON_KEY est manquant.')
  }
...
export const gamesService = {
  ...
  removeGameChannel(channel, gameId) {
    if (!USE_SUPABASE || !supabase || !channel) return

    supabase.removeChannel(channel)
    if (gameId && localDB.channels?.[gameId] === channel) {
