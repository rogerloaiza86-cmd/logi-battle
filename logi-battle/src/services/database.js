...
const getSupabaseClient = () => {
  if (!isSupabaseConfigured || !supabase) {
    throw new Error('VITE_DB_MODE=supabase requiert VITE_SUPABASE_URL et VITE_SUPABASE_ANON_KEY')
  }
  return supabase
}
...
export const gamesService = {
  ...
  getGameChannel(gameId) {
    if (USE_SUPABASE) {
      ...
      if (!localDB.channels[gameId]) {
        localDB.channels[gameId] = getSupabaseClient().channel(`game_${gameId}`)
      }
  ...
  removeGameChannel(gameId) {
    if (USE_SUPABASE && localDB.channels?.[gameId]) {
      getSupabaseClient().removeChannel(localDB.channels[gameId])
